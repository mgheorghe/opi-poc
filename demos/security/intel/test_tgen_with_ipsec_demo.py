...
def dpu():
    ...
    dpu_connect.send_command('ip addr add 200.0.0.1/24 dev enp0s1f0d1')
    dpu_connect.send_command('ip addr add 201.0.0.1/24 dev enp0s1f0d2')
    ...
    dpu_connect.send_command('ip addr del 200.0.0.1/24 dev enp0s1f0d1')
    dpu_connect.send_command('ip addr del 201.0.0.1/24 dev enp0s1f0d2')
...
def test_server_to_server_via_ipsec_and_dpu(dpu, server):
    ...
    tgen = snappi.api(location=f'https://{TGEN1_IP}', verify=False)
    cfg = tgen.config()
